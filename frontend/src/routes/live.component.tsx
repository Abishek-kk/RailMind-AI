...
import { riskColor, type BoundingBox, type RiskLevel } from "@/lib/mock-data";
...
export default function LivePage() {
  ...
  // Use a Map instead of a Set so insertion order is tracked for O(1) eviction
  // of the oldest entry once the cap is hit — prevents a slow memory leak on
  // long-running 24/7 sessions.
  ...
  const playNotificationSound = useCallback(() => {
    ...
    const now = Date.now();
    if (now - lastAudioPlaybackAt.current < 300) {
      return;
    }
  ...
  const showAlertToast = useCallback(
    (alert: ApiAlert) => {
      ...
      // Evict the oldest entry if we've hit the cap, then record the new ID.
  ...
  const realFeeds = useMemo(() => (Array.isArray(feeds) ? feeds : []), [feeds]);
              ...
              {filteredFeeds.length === 0 ? (
                <div className="rounded-xl border border-border bg-card p-8 text-center">
                  ...
                  <p className="mt-3 text-sm text-muted-foreground">
                    No active CCTV feeds. Add a feed to begin monitoring.
                  </p>
                </div>
              ) : (
