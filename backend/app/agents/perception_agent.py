...
def perception_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Ingests raw Computer Vision and transformer data and structures it into a clear observation.
    """
    raw_data = state.get("raw_data", {})
    
    transformer_scores = raw_data.get("transformer_scores", {})
    transformer_score = raw_data.get("transformer_score")
    if transformer_score is None:
        transformer_score = max(transformer_scores.values()) if transformer_scores else raw_data.get("transformer_anomaly_score", 0.0)
    transformer_anomaly_score = raw_data.get(
    ...
    structured_observation = {
        ...
        "transformer_anomaly_score": transformer_anomaly_score,
