...
class RiskScorer:
    ...
    def calculate(self, incident_attributes):
        ...
        score += lstm_score * weights.get("lstm", 0.4)
        ...
        score += following_risk * weights.get("following", 0.1)
        ...
        score += pose_risk * weights.get("pose", 0.1)

        context_multiplier = float(incident_attributes.get("context_multiplier", 1.0))
        score += min(1.0, context_multiplier * weights.get("context", 0.1))
